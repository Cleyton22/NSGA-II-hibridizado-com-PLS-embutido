...
Solution* mutation(Solution *solutionM, Edge *allEdges, int numVertices){
    ...
    Solution *newSolution = copySolution(solutionM);
...
// Função para criar uma população de soluções não dominadas para usar como entrada para o PLS semi-paralelo
List* createNonDominatedPopulationList(Population *initialOffsprings) {
    List *nonDominatedSolutions = createList();
    Node *lastNode = NULL;

    int accepted = 0;
    for (int i = 0; i < initialOffsprings->numSolutions; i++){
        accepted = 0;
        for(int j = 0; j < initialOffsprings->numSolutions; j++){
            if(i != j){
                if (dominates(&initialOffsprings->solutions[j], &initialOffsprings->solutions[i])){
                    accepted = 1;
                }
            }
        }
        ...
        if(accepted != 1){
            Solution *acceptedSolution = copySolution(&initialOffsprings->solutions[i]);
...
// Função para executar os passos principais do PLS semi-paralelo
List* paretoLocalSearchAdaptedToNsga2(Population *initialOffsprings, Edge *allEdges, int numVertices){
    List *unexploredSolutions = createNonDominatedPopulationList(initialOffsprings);
    ...
    Node *lastNode = NULL;
    for(Node *node = unexploredSolutions->head; node != NULL; node = node->next){
        lastNode = addToList2(allNonDominatedSolutions, node->solution, lastNode);
    }
    ...
    do{
        ...
        removeFromList(unexploredSolutions, current->solution);

        freeList(temporaryNeighborsSolutions);

    } while(unexploredSolutions->size != 0);

    freeList(unexploredSolutions);
...
// Função secundária para executar os passos principais do NSGA-II com o PLS semi-paralelo até o critério de parada
Population* secondaryMainLoopToNsga2WithPls(Population *nextGeneration, ListDoubly *nonDominatedSolutions, Edge *allEdges, int numVertices){
    ...
    List *plsOffspringList = paretoLocalSearchAdaptedToNsga2(initialOffsprings, allEdges, numVertices);

    Population *plsOffspringsResult = malloc(sizeof(Population));
    plsOffspringsResult->solutions = malloc(plsOffspringList->size * sizeof(Solution));

    int plsPopulationSize = 0;
    for (Node *node = plsOffspringList->head; node != NULL; node = node->next){
        plsOffspringsResult->solutions[plsPopulationSize] = *copySolution(node->solution);
        plsPopulationSize++;
    }

    plsOffspringsResult->numSolutions = plsPopulationSize;
    freeList(plsOffspringList);

    for (int i = 0; i < plsOffspringsResult->numSolutions; i++){
        addNonDominatedSolutionsToParetoFront(nonDominatedSolutions, &plsOffspringsResult->solutions[i]);
    }

    Population *combinedPopulation = malloc(sizeof(Population));
    combinedPopulation->solutions = malloc((POPULATION_SIZE + plsOffspringsResult->numSolutions) * sizeof(Solution));
    ...
    for(int j = 0; j < plsOffspringsResult->numSolutions; j++){
        combinedPopulation->solutions[combinedPopulation->numSolutions] = *copySolution(&plsOffspringsResult->solutions[j]);
        combinedPopulation->numSolutions++;
    }
    ...
    freePopulation(plsOffspringsResult);
...
// Função primária para executar os passos principais do NSGA-II com o PLS semi-paralelo até o critério de parada
void nsga2WithPlsMainLoop(Population *initialPopulation, Edge *allEdges, int numVertices){
    ...
    printf("Quantidade de solucoes encontradas (NSGA-II e PLS embutido): %d\n\n", nonDominatedSolutions->size);

    cpuTimeUsed = ((double) (end - start - (pauseEnd - pauseStart))) / CLOCKS_PER_SEC;
    printf("NSGA-II hibridizado com PLS embutido levou %f segundos para executar \n", cpuTimeUsed);
}
